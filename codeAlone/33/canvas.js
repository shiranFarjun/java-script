...
function draw({ key }) {          // write a draw function
    hue += 1;                     // increment the hue
    ctx.strokeStyle = `hsl(${Math.random() * 360}, 100%, 50%)`;
    console.log(key);
    ctx.beginPath();              // start the path
    ctx.moveTo(x, y);
    switch (key) {                // move our x and y values depending on what the user did
        case 'ArrowUp':
            y -= MOVE_AMOUNT;
            break;
        case 'ArrowRight':
...
function handleKey(e) {                // write a handler for the keys
...
function clearCanvas() {              // clear -> shke function
